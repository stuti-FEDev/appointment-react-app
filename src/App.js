...
function App() {
  ...
  const filteredAppointment = appointmentList.filter(
    ...
    return(
      a[sortBy].toLowerCase() < b[sortBy].toLowerCase() ? -1*order : 1*orderBy
    )
  ...
  return (
    <div className="App container mx-auto mt-3 font-thin">
      <h1 className="text-5xl mb-3"> <BiCalendar className="inline-block text-red-400 align-top" />Your Appointment </h1>
        <AddAppointemnt onSendAppointment = {myAppointment => setAppointmentList([...appointmentList,myAppointment])}
        lastId={appointmentList.reduce((max,item)=>Number(item.id) > max ? Number(item.id) : max, 0)}/>
        <Search query={query} 
        searchAppointment={myQuery => setQuery(myQuery)} 
        sortBy={sortBy} 
      ...
      <ul className="divide-y divide-gray-200">
        {
          filteredAppointment.map( appointments => (
            <AppointmentInfo  key={appointments.id} appointments = {appointments} deleteAppointment={deleteAppointment}/>
          ))
        }
      </ul>
    </div>
